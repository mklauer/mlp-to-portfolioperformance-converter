...
CATEGORY_TO_TYPE = { 'Zinseinkünfte' : 'Zinsen',
...
def search_header(csv_file):
    ...
    for line in csv_file:
        ...
        if 'Valuta' in line and 'Umsatz' in line:
            found_header = line
            break
...
def main():
    ...
    arg_parser.add_argument("-o", "--outfile", help="Name of the converted output CSV file")
    ...
    output_file = command_args.outfile if command_args.outfile else f"{os.path.splitext(input_file)[0]}_converted.csv"

    csv_input = fileinput.input(files=input_file, mode='r', openhook=opening_hook_csv)
    header, header_offset = search_header(csv_input)
    transaction_reader = csv.DictReader(csv_input, fieldnames=header, delimiter=';', quotechar='"')

    field_names = ['Datum', 'Typ', 'Wert', 'Buchungswährung', 'Steuern', 'Stück', 'ISIN', 'WKN', 'Ticker-Symbol',
                   'Wertpapiername', 'Notiz']
    csv_output = open(output_file, 'w', newline='')
    transaction_writer = csv.DictWriter(csv_output, fieldnames=field_names, delimiter=';')
    transaction_writer.writeheader()

    row_cnt = process_transactions(transaction_reader, transaction_writer, header_offset)
